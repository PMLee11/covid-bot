...
# Load data once when app starts - relative path for deployment
df = pd.read_csv('worldometer_data.csv')
print(f"Data loaded: {df.shape[0]} rows, {df.shape[1]} columns")
print(f"Columns: {list(df.columns)}")

# Groq API configuration - reads from environment variable
...
def create_visualization(result_data, question):
...
def ask_question():
    ...
    try:
        ...
        print(f"Error executing code: {code if 'code' in locals() else 'N/A'}")
        return jsonify({
            'success': False,
            'question': question,
